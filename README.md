...
-   Map предстовляет собой тип данных который является словарем. Работает по концепции ключ значение. Ключ в map должен быть уникальным (не должен дублироваться)
...
Если ключа к котому мы обращаемся нет, то будет выведено значение по умолчанию. Например:

```go
fmt.Println(users["Vova"]) // Вывод: 0
```

Так же в map мы можем проверить, есть ли элемент. Нужно просто передать вторую переменную, которую обычно называют exists или ok. Если значение есть, то переменная будет true, если нет, то false. Например:
...
По map так же можно итерироваться как со слайс:
